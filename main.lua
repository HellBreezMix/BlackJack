...
-- Чтобы require("config") работал из любой папки
...
function Players.get(name)
    ...
    if not Players.data[name] then
        Players.data[name] = {
            balance     = 0,
            totalPlayed = 0,  -- сумма всех сделанных ставок (наигранные ЭМ)
            totalWon    = 0,  -- сумма выигранных ЭМ
            games       = 0,
            wins        = 0
        }
        Players.save()
...
-- Добавить к "наигранным" (вызывается при каждой ставке)
...
function Players.getTop(n)
    ...
    for name, data in pairs(Players.data) do
        table.insert(list, {
            name    = name,
            total   = data.totalPlayed or 0,  -- топ по наигранным
...
--------------------------------------------------
-- НАСТРОЙКИ (мин/макс ставка, цены скупки)
--------------------------------------------------
local Settings = {
    data = {}
}
...
function Settings.load()
    local def = {
        ...
        buyPrices = deepCopy(config.buyPrices)
    }
    Settings.data = loadDB(config.paths.settings) or def
    ...
    Settings.data.buyPrices = Settings.data.buyPrices or deepCopy(config.buyPrices)
end
...
-- Получить стак из сундука ставки (слот 1)
...
-- Забрать предметы из сундука ставки.
-- Сначала пробуем закинуть в ME сеть (если есть интерфейс),
-- иначе просто перекидываем в инвентарь компьютера (side 0).
function Hardware.consumeDeposit(count)
    if not Hardware.transposer then return false end
    count = count or 64

    -- Попытка импорта в ME через интерфейс (если компьютер стоит рядом)
    if Hardware.me then
        -- ME Interface может забрать предметы со стороны
        local ok = pcall(function()
            -- некоторые версии AE2 OC имеют importItem
            if Hardware.me.importItem then
                Hardware.me.importItem(config.hardware.transposerSide, 1, count)
            end
        end)
        if ok then return true end
    end

    -- Fallback: перекинуть в компьютер
    local ok = pcall(Hardware.transposer.transferItem, config.hardware.transposerSide, 0, count, 1)
    return ok
end

-- Экспорт из ME в сундук выплаты (если понадобится физическая выдача)
function Hardware.exportFromME(filter, count)
    if not Hardware.me then return 0 end
    local ok, moved = pcall(Hardware.me.exportItem, filter, config.hardware.meSide, count or 1)
...
local Game = {
    deck     = {},
    player   = { hand = {}, standing = false },
    dealer   = { hand = {}, standing = false },
    finished = false,
    result   = nil,          -- "WIN", "LOSE", "DRAW", "BLACKJACK"
    bet      = 0,
    state    = "idle"        -- idle | betting | playing | result
}
...
function Game.deal()
    ...
    if Cards.isBlackjack(Game.player.hand) then
        if Cards.isBlackjack(Game.dealer.hand) then
            Game.finish("DRAW")
        else
            Game.finish("BLACKJACK")
        end
...
function Game.stand()
    ...
    if Cards.isBust(Game.dealer.hand) then
        Game.finish("WIN")
    elseif p > d then
        Game.finish("WIN")
    elseif p < d then
...
local UI = {
    w = 0, h = 0,
    screen = "main",           -- main | playing | result | admin | admin_buy | admin_add_item | admin_edit_item
    playerName = nil,
    authorized = false,
    sessionLeft = 120,         -- секунд
    timerId = nil,
    ...
    -- Админ
    adminTab = "bets",         -- bets | buy
    -- Добавление/редактирование предмета
    editItem = {
        name     = nil,        -- id предмета
        label    = "",         -- отображаемое имя
        price    = "1",        -- строка ввода цены
        mode     = "add"       -- add | edit
    }
}
...
function UI.drawButton(b)
    ...
    gpu.setForeground(b.fg or config.colors.text)
...
local function centerText(y, str, fg, bg, width)
    width = width or UI.w
    local x = math.floor((width - unicode.len(str)) / 2) + 1
    text(x, y, str, fg, bg)
end

-- Пиксельные раскладки мастей на карте (из оригинала, адаптировано)
local faceLayout = {
    ["A"]  = {{4,3}},
    ["2"]  = {{4,1},{4,5}},
    ["3"]  = {{4,1},{4,3},{4,5}},
...
local function drawCard(x, y, card, hidden)
    local cw, ch = 9, 7   -- чуть крупнее для пикселей
    if hidden then
    ...
    if type(layout) == "string" then
        ...
        -- Числовые + туз: рисуем масти по координатам
...
local function drawHand(x, y, hand, hideFirst)
    ...
    for i, c in ipairs(hand) do
        drawCard(x + (i - 1) * (cardW + 1), y, c, hideFirst and i == 1)
    end
end
...
function UI.drawSidebar()
    ...
    if not UI.authorized then
        UI.addButton(sx + 1, 3, sw - 2, 3, "АВТОРИЗАЦИЯ", config.colors.buttonGreen, 0xFFFFFF, function()
            -- авторизация происходит по touch player
        end, "auth")
    ...
    for name, price in pairs(prices) do
        ...
        text(sx + 1, y, string.format("%s - %s %s", short, price, config.currency.symbol), config.colors.text, config.colors.panel)
        y = y + 1
    end
    if count == 0 then
    ...
    for i, entry in ipairs(top) do
        ...
        local balStr
        if bal >= 1000 then
            balStr = string.format("%.1fk", bal / 1000)
        else
            balStr = tostring(bal)
        end
...
function UI.drawMainArea()
    ...
    if UI.screen == "main" then
        ...
        if UI.authorized then
            ...
            centerText(10, "Нажмите АВТОРИЗАЦИЯ", config.colors.textDark, config.colors.background, mw)
            centerText(12, "и коснитесь экрана своим ником", config.colors.textDark, config.colors.background, mw)
        end
    ...
    elseif UI.screen == "playing" or UI.screen == "result" then
        ...
        -- Игрок
        text(4, 12, "ВЫ", config.colors.text, config.colors.background)
        text(10, 12, tostring(Cards.handValue(Game.player.hand)), config.colors.textGold, config.colors.background)
        drawHand(4, 14, Game.player.hand, false)

        -- Ставка
        text(4, 21, "Ставка: " .. Game.bet .. " " .. config.currency.symbol, config.colors.text, config.colors.background)

        if UI.screen == "playing" and not Game.finished then
            UI.addButton(4, 23, 12, 3, "ВЗЯТЬ", config.colors.buttonGreen, 0xFFFFFF, function()
                Game.hit()
            ...
            UI.addButton(18, 23, 12, 3, "СТОП", config.colors.buttonRed, 0xFFFFFF, function()
                Game.stand()
                UI.resolveGame()
                UI.draw()
            end)
        elseif UI.screen == "result" then
            local resText = ({
                ...
                DRAW = "НИЧЬЯ",
                BLACKJACK = "BLACKJACK!"
            })[Game.result] or Game.result

            local resCol = ({
                WIN = config.colors.textGreen,
                LOSE = config.colors.textRed,
                DRAW = config.colors.textGold,
                BLACKJACK = config.colors.textGold
            })[Game.result] or config.colors.text

            centerText(22, resText, resCol, config.colors.background, mw)

            local mult = Game.payoutMultiplier()
            local winAmount = math.floor(Game.bet * mult + 0.5)
            if winAmount > 0 then
                centerText(23, "+" .. winAmount .. " " .. config.currency.symbol, config.colors.textGreen, config.colors.background, mw)
            end

            UI.addButton(math.floor(mw/2) - 8, 25, 16, 3, "ЕЩЁ РАЗ", config.colors.buttonGreen, 0xFFFFFF, function()
                UI.screen = "main"
                Game.reset()
                UI.draw()
            end)
...
function UI.drawAdmin(mw)
    ...
    UI.addButton(4, tabY, tabW, 3, "СТАВКИ", 
        UI.adminTab == "bets" and config.colors.buttonBlue or config.colors.button,
        0xFFFFFF, function()
            UI.adminTab = "bets"
            UI.draw()
        end)
    UI.addButton(4 + tabW + 2, tabY, tabW, 3, "СКУПКА",
        UI.adminTab == "buy" and config.colors.buttonBlue or config.colors.button,
        0xFFFFFF, function()
            UI.adminTab = "buy"
            UI.draw()
        end)

    if UI.adminTab == "bets" then
        UI.drawAdminBets(mw)
    else
        UI.drawAdminBuy(mw)
    end
...
function UI.drawAdminBets(mw)
    ...
    text(4, 11, tostring(Settings.data.minBet) .. " " .. config.currency.symbol, config.colors.textGold, config.colors.background)

    UI.addButton(28, 10, 5, 3, "−", config.colors.button, config.colors.text, function()
        Settings.data.minBet = math.max(1, Settings.data.minBet - 1)
        Settings.save()
        UI.draw()
    end)
    UI.addButton(34, 10, 5, 3, "+", config.colors.button, config.colors.text, function()
        Settings.data.minBet = Settings.data.minBet + 1
    ...
    text(4, 15, "Максимальная ставка", config.colors.textDark, config.colors.background)
    text(4, 16, tostring(Settings.data.maxBet) .. " " .. config.currency.symbol, config.colors.textGold, config.colors.background)

    UI.addButton(28, 15, 5, 3, "−", config.colors.button, config.colors.text, function()
        Settings.data.maxBet = math.max(Settings.data.minBet, Settings.data.maxBet - 1)
        Settings.save()
        UI.draw()
    end)
    UI.addButton(34, 15, 5, 3, "+", config.colors.button, config.colors.text, function()
        Settings.data.maxBet = Settings.data.maxBet + 10
        Settings.save()
        UI.draw()
    end)
end
...
--------------------------------------------------
-- ЭКРАН: Добавление предмета (шаг 1 — ждать предмет)
--------------------------------------------------
...
--------------------------------------------------
-- ЭКРАН: Редактирование названия и цены
--------------------------------------------------
function UI.drawAdminEditItem(mw)
    ...
    -- Поле имени (кликабельное — ввод через клавиатуру)
    ...
    UI.addButton(4, 12, 40, 1, "", 0x000000, 0x000000, function()
        UI.inputText("Имя предмета", UI.editItem.label, function(val)
            if val and val ~= "" then
                UI.editItem.label = val
            end
            UI.draw()
        end)
    end)
    ...
    UI.addButton(4, 16, 20, 1, "", 0x000000, 0x000000, function()
        UI.inputText("Цена", UI.editItem.price, function(val)
    ...
    UI.addButton(4, 20, 16, 3, "СОХРАНИТЬ", config.colors.buttonGreen, 0xFFFFFF, function()
        ...
        Settings.data.buyPrices[UI.editItem.name] = price
        -- Можно хранить и label, если захотим
        Settings.save()
...
-- Простой ввод текста с клавиатуры (временно переключаемся в term)
function UI.inputText(title, default, callback)
    ...
    term.setCursor(1, 1)
    print("=== " .. title .. " ===")
    print("Текущее: " .. tostring(default or ""))
    print("")
    print("Введи новое значение и нажми Enter")
    print("(пустая строка = отмена)")
    term.write("> ")
...
function UI.login(name)
    ...
    UI.screen = "main"
    Players.get(name) -- создаём если нет
...
function UI.startSessionTimer()
    ...
    UI.timerId = event.timer(1, function()
        ...
        -- обновляем только таймер в сайдбаре (полная перерисовка каждый раз проста)
        UI.draw()
    end, math.huge)
end
...
function UI.doDeposit()
    ...
    log(string.format("Deposit %s: %s x%d = %d ЭМ", UI.playerName, item.name, item.size, total))
    UI.draw()
end
...
function UI.startGame()
    ...
    -- Списываем ставку и добавляем в "наигранные"
...
local function boot()
    ...
    gpu.setResolution(math.min(100, maxW), math.min(30, maxH))
    ...
    log("BlackJack " .. config.project.version .. " started")

    UI.draw()

    -- Главный цикл
    while true do
        local ev = { event.pull(0.5) }  -- таймаут чтобы таймер сессии работал
        local e = ev[1]
        ...
        if e == "touch" then
            ...
            if not UI.authorized then
                -- Любое касание при неавторизованном = попытка входа
